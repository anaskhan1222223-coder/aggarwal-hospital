...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      ...
      {/* Main bar */}
      <nav className={`transition-all duration-300 ${scrolled ? "border-b border-[#DCE4EC] bg-white/95 py-2.5 backdrop-blur" : "bg-transparent py-4"}`}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <a href={callLink()} aria-label="Call hospital" className={`rounded-lg border p-2.5 lg:hidden ${scrolled ? "border-[#DCE4EC] text-[#0E3A35]" : "border-white/30 text-white"}`}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.36 1.9.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.34 1.85.57 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
            </a>
            <button onClick={() => setOpen(!open)} aria-label="Menu" aria-expanded={open} className={`rounded-lg border p-2.5 lg:hidden ${scrolled ? "border-[#DCE4EC] text-[#0E3A35]" : "border-white/30 text-white"}`}>
      ...
      {/* Mobile menu — clean white, large targets */}
      {open && (
        <div className="fixed inset-0 top-0 z-[60] flex flex-col bg-white lg:hidden">
          <div className="flex items-center justify-between border-b border-[#DCE4EC] px-5 py-4">
            ...
            <button onClick={() => setOpen(false)} aria-label="Close menu" className="rounded-lg border border-[#DCE4EC] p-2.5 text-[#0E3A35]">
